    ...
    .addPlugin(
        new webpack.DefinePlugin({
            ...
            "process.env.ASSET_HOST": `"${
                process.env.ASSET_HOST || "https://assets.scratch.mit.edu"
            }"`,
        })
    )
    .addPlugin(
...
const distStandaloneConfig = baseConfig.clone().merge({
    entry: {
        "scratch-gui-standalone": path.join(
            __dirname,
            "src/index-standalone.tsx"
        ),
    },
    output: {
        path: path.resolve(__dirname, "dist"),
...
const buildConfig = baseConfig
    ...
    .merge({
        entry: {
            ...
            vmplayer: "./src/playground/vm-player.jsx",
        },
        output: {
    ...
    .addPlugin(
        new HtmlWebpackPlugin({
            ...commonHtmlWebpackPluginOptions,
            chunks: ["vmplayer"],
            filename: "vm-player.html",
            template: "src/playground/index.ejs",
            title: "Multiplayer Game Stream",
        })
    )
    .addPlugin(
        new CopyWebpackPlugin({
...
switch (process.env.BUILD_TYPE) {
    case "dist":
        config = distConfig.get();
        break;
    case "dist-standalone":
        config = distStandaloneConfig.get();
        break;
    default:

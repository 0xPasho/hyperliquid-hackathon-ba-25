...
const ProjectSaverHOC = function (WrappedComponent) {
    class ProjectSaverComponent extends React.Component {
        ...
        storeProject (projectId, requestParams, options) {
            ...
            // Use API to save/update projects
            const saveProjectToAPI = async (id, vmState, params) => {
                log.info(`💾 Saving project to API: ${id ? `updating ${id}` : 'creating new'}`);

                if (id) {
                    // Update existing project
                    return APIClient.updateProject(id, {
                        title: params.title || this.props.reduxProjectTitle,
                        projectData: vmState
                    });
                } else {
                    // Create new project
                    return APIClient.createProject({
                        title: params.title || this.props.reduxProjectTitle || 'Untitled',
                        projectData: vmState
                    });
                }
            };

            const saveProject = this.props.onUpdateProjectData || saveProjectToAPI;

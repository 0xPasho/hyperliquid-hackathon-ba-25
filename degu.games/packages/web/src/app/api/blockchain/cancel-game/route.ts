...
/**
 * POST /api/blockchain/cancel-game
 *
 * Cancel a betting game (creator only, only if open)
 */
export async function POST(request: NextRequest) {
    try {
        console.log('[Next.js API] Cancelling game...');
        ...
        const response = await fetch(`${BACKEND_API_URL}/blockchain/betting/cancel-game`, {
            method: 'POST',
            headers: {
        ...
        console.log('[Next.js API] ✅ Game cancelled successfully');
        return NextResponse.json(result);

    } catch (error) {
        console.error('[Next.js API] Error cancelling game:', error);
        return NextResponse.json(
            {
                success: false,

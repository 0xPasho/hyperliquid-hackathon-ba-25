...
export async function POST(request: NextRequest) {
    try {
        console.log('[Next.js API] Transferring tokens...');
        ...
        const response = await fetch(`${BACKEND_API_URL}/blockchain/transfer`, {
            method: 'POST',
            headers: {
        ...
        console.log('[Next.js API] ✅ Transfer successful');
        return NextResponse.json(result);

    } catch (error) {
        console.error('[Next.js API] Error transferring tokens:', error);
        return NextResponse.json(
            {
                success: false,

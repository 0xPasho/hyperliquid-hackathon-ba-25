...
/**
 * POST /api/blockchain/select-winners
 *
 * Select winners for a betting game (creator only)
 */
export async function POST(request: NextRequest) {
    try {
        console.log('[Next.js API] Selecting winners...');
        ...
        if (gameId === undefined || !winners || !Array.isArray(winners)) {
            return NextResponse.json(
                { success: false, error: 'Game ID and winners array are required' },
                { status: 400 }
            );
        }
        ...
        const response = await fetch(`${BACKEND_API_URL}/blockchain/betting/select-winners`, {
            method: 'POST',
            headers: {
        ...
        console.log('[Next.js API] ✅ Winners selected successfully');
        return NextResponse.json(result);

    } catch (error) {
        console.error('[Next.js API] Error selecting winners:', error);
        return NextResponse.json(
            {
                success: false,

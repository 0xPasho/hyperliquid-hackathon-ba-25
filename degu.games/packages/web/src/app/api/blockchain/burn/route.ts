...
/**
 * POST /api/blockchain/burn
 *
 * Burn DEGU tokens
 */
export async function POST(request: NextRequest) {
    try {
        console.log('[Next.js API] Burning tokens...');
        ...
        const response = await fetch(`${BACKEND_API_URL}/blockchain/burn`, {
            method: 'POST',
            headers: {
        ...
        console.log('[Next.js API] ✅ Tokens burned successfully');
        return NextResponse.json(result);

    } catch (error) {
        console.error('[Next.js API] Error burning tokens:', error);
        return NextResponse.json(
            {
                success: false,

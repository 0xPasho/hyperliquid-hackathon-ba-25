...
/**
 * POST /api/blockchain/start-game
 *
 * Start a betting game (creator only)
 */
export async function POST(request: NextRequest) {
    try {
        console.log('[Next.js API] Starting betting game...');
        ...
        const response = await fetch(`${BACKEND_API_URL}/blockchain/betting/start-game`, {
            method: 'POST',
            headers: {
        ...
        console.log('[Next.js API] ✅ Game started successfully');
        return NextResponse.json(result);

    } catch (error) {
        console.error('[Next.js API] Error starting game:', error);
        return NextResponse.json(
            {
                success: false,

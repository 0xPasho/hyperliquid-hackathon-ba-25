...
wss.on('connection', (ws, req) => {
  ...
  function performHandshake(roomId, username) {
    ...
    // Track player for betting handler
  ...
  function performSet(variable, value) {
    ...
    // Notify betting handler of variable change
  ...
  ws.on('close', (code) => {
    ...
    // Remove player from betting handler
    if (client.room && client.username) {
      bettingHandler.removePlayer(client.room.id, client.username);
    }
